<?php
namespace Eastlane\PaysonCheckout2\Observer\Sales;
...
class RefundPaysonAmount implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        if($this->paysonConfig->getCaptureInMagento()) {
            ...
            if ($order->getPayment()->getMethod() == \Eastlane\PaysonCheckout2\Model\Paysoncheckout2ConfigProvider::CHECKOUT_CODE) {
                if ($checkout->status == 'shipped') {
                    try{
                        ...
                        foreach($checkout->payData->items as $paysonItem)
                        {
                            ...
                            if($paysonItem->type == \Eastlane\PaysonCheckout2\Helper\Order::FEE) {
