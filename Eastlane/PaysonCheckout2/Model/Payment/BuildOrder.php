    ...
    */
    ...
    protected  function _savePaymentMethod($quote)
    {
        try{
            ...
            $payment->setMethod(\Eastlane\PaysonCheckout2\Model\Paysoncheckout2ConfigProvider::CHECKOUT_CODE);
            $payment->save();
        } catch (\Exception $e){
            $paysonLoggerHelper  = $this->_paysonHelper;
            $paysonLoggerHelper->error($e->getMessage());
    ...
    protected function _saveQuoteCheckoutId($quote, $checkoutId)
    {
        try{
            /* quote save */
            $quote->setData(\Eastlane\PaysonCheckout2\Model\ConfigInterface::CHECKOUT_ID_COLUMN, $checkoutId);
            $quote->save();
        } catch (\Exception $e){
            $paysonLoggerHelper  = $this->_paysonHelper;
            $paysonLoggerHelper->error($e->getMessage());
